...
static int is_classloader_init = FALSE;
static int is_classht_init = FALSE;
...
static void setIndexedAttributeData(AttributeData ***attributes,
                                    ...
                                    int size) {
    if(*attributes == NULL) {
        *attributes = sysMalloc_persistent(size * sizeof(AttributeData*));
        memset(*attributes, 0, size * sizeof(AttributeData*));
    }

    (*attributes)[index] = sysMalloc_persistent(sizeof(AttributeData));
...
#define setSingleAttribute(attributes, pntr, length) \
    attributes = sysMalloc_persistent(sizeof(AttributeData));   \
    attributes->len = length;                        \
    attributes->data = sysMalloc_persistent(length);            \
...
Class *parseClass(char *classname, char *data, int offset, int len,
    ...
    interfaces = classblock->interfaces =
                         sysMalloc_persistent(intf_count * sizeof(Class *));
    ...
    classblock->fields = sysMalloc_persistent(classblock->fields_count *
                                   sizeof(FieldBlock));
        ...
        for(; attr_count != 0; attr_count--) {
            ...
            if(attr_name == SYMBOL(Code)) {
                ...
                method->exception_table =
                        sysMalloc_persistent(method->exception_table_size *
                ...
                for(; code_attr_cnt != 0; code_attr_cnt--) {
                    ...
                    if(attr_name == SYMBOL(LineNumberTable)) {
                        ...
                        method->line_no_table =
                                sysMalloc_persistent(method->line_no_table_size *
                ...
                method->throw_table = sysMalloc_persistent(method->throw_table_size *
                                                sizeof(u2));
    ...
    for(; attr_count != 0; attr_count--) {
        ...
        if(attr_name == SYMBOL(SourceFile)) {
            ...
            READ_U2(size, ptr, len);
            {
                ...
                if(classblock->inner_class_count) {
                    classblock->inner_classes =
                                sysMalloc_persistent(classblock->inner_class_count *
                                          sizeof(u2));
            ...
            data = sysMalloc_persistent(attr_length + num_methods*2 + 2);
    ...
    if(i < sizeof(ExtraAttributes)/sizeof(void*)) {
        classblock->extra_attributes = sysMalloc_persistent(sizeof(ExtraAttributes));
...
Class *defineClass(char *classname, char *data, int offset, int len,
    ...
    if(class != NULL) {
        ...
        //jam_printf("<defineclass class:%p found:%p >\n",class,found);
        if(found != class) {
            //jam_printf("<defineclass: Try to reload %s>\n",classname);
            if(class_loader != NULL) {
...
void linkClass(Class *class) {
   ...
   ClassBlock *cb = CLASS_CB(class);
   //if(strcmp(cb->name,"java/lang/VMObject") == 0) jam_printf("<Class: linking %s>\n",cb->name);
   ...
   if(cb->state >= CLASS_LINKED){
     //if(!is_first_exp()) jam_printf("<Class: Class linked name:%s %d>\n",cb->name,cb->state);
     return;
   }
   ...
   for(mb = cb->methods, i = 0; i < cb->methods_count; i++,mb++) {
       ...
       if(mb->access_flags & ACC_NATIVE) {
           ...
           newNativeMethodUtf8(CLASS_CB(mb->class)->name);

           //jam_printf("<Class: Class %s has nativemethod %s>\n",CLASS_CB(mb->class)->name,mb->name);
   ...
   cb->imethod_table = sysMalloc_persistent((spr_imthd_tbl_sze + new_itable_count) *
                                 sizeof(ITableEntry));
   ...
   if(!(cb->access_flags & ACC_INTERFACE)) {
       ...
       if(miranda_count > 0) {
           ...
           mb = sysRealloc_persistent(cb->methods, (cb->methods_count + miranda_count)
                                        * sizeof(MethodBlock));
           ...
           if(new_mtbl_count > 0) {
               ...
               method_table = sysRealloc_persistent(method_table, method_table_size *
                                                       sizeof(MethodBlock*));
           }
   ...
   if(cb->name == SYMBOL(java_lang_ClassLoader)) {
       if(verbose) jam_printf("classlibCacheClassLoaderFields\n");
...
Class *initClass(Class *class) {
   ...
   if(cb->state >= CLASS_INITED){
     if(!is_first_exp()){
       //jam_printf("<Class: Class inited name:%s %d>\n",cb->name,cb->state);
     }
     return class;
   }

   //if(!is_first_exp()) jam_printf("<Class: Try to relink %s %d>\n",cb->name,cb->state);
   linkClass(class);
   objectLock(class);
   ...
   if(cb->state == CLASS_BAD) {
       jam_printf("<Class: Class is Bad %s>\n",cb->name);
       objectUnlock(class);
...
Class *findHashedClass(char *classname, Object *class_loader) {
    ...
    if(class_loader == NULL){
        ...
        if(is_classht_init == FALSE){
          if((table = classlibCreateLoaderTable(class_loader)) == NULL) return NULL;
          is_classht_init = TRUE;
          if(is_hash_exist == FALSE) return NULL;
        }
        else if((table = classlibLoaderTable(class_loader)) == NULL){
          return NULL;
        }
    }
...
#define COMPARE(ptr1, ptr2, hash1, hash2) (hash1 == hash2) && \
    ...
    if(bootc){
      //jam_printf("Try to find Hased class:%s,table:%p\n",name,class,table);
      findHashEntry((*table), name, class, FALSE, FALSE, TRUE, boot_name, FALSE );
    }else{
      //jam_printf("Try to find Hased class:%s,table:%p\n",name,table);
      findHashEntry((*table), name, class, FALSE, FALSE, TRUE, class_name, FALSE );
    }
...
Class *findSystemClass0(char *classname) {
   ...
   //if(exceptionOccurred()) printf("exception occured at find class:%p name:%s\n",class,classname);
   if(!exceptionOccurred())
       linkClass(class);
...
Class *findSystemClass(char *classname) {
   //if(testing_mode) jam_printf("try to find %s\n",classname);
...
Class *findArrayClassFromClassLoader(char *classname, Object *class_loader) {
   ...
   //if(testing_mode) jam_printf("class:%s addr:%p\n",classname,class);
   if(class == NULL)
    ...
    if(class == NULL) {
        ...
        if((excep = exceptionOccurred()) || class == NULL) {
            ...
            signalChainedException(java_lang_NoClassDefFoundError,
                                   classname, excep);
            if(testing_mode) jam_printf("java_lang_NoClassDefFoundError:%p\n",class);
            return NULL;
        }
...
int reinitClass(char *name){
  ...
  if(!class){
    ...
    if(!class){
      jam_printf("<Class: Class not found:%s>\n",name);
      return FALSE;
    }
  }
  ClassBlock *cb = CLASS_CB(class);
  cb->state = CLASS_LOADED;
  clearException();
  linkClass(class);
...
int reinitialiseNativeMethods(){
  if(is_first_exp()) return TRUE;
  char *name = NULL;
  getHashedNativeMethodUtf8();
  if(!reinitClass("java/lang/Runtime")) return FALSE;
  return TRUE;
}
...
Object *getSystemClassLoader() {
    ...
    if(!exceptionOccurred()) {
      //jam_printf("<system class loader execute ...>\n");
        ...
        if(mb != NULL) {
            ...
            //jam_printf("<system class loader executed.%p>\n",class_loader);
            if(!exceptionOccurred())
                return loader;
        }
    }
...
int recoveryClass(Object *root){
    ...
    ClassBlock *cb = CLASS_CB(root->class);
    jam_printf("classname:%s,%p,classaddr:%p\n",cb->name,cb->name,root->class);
    for(i = 0; i < counts; i++){
      cb->fields[i].u = old_cb->fields[i].u;
    }
    gcPendingFree(old_class);
    return TRUE;
}
...
#define COMPARE(ptr1, ptr2, hash1, hash2) (hash1 == hash2 && \
    ...
    findHashEntry(boot_packages, package_name, hashed, FALSE, FALSE, TRUE, bootp_name, FALSE);
...
Object *bootClassPathResource(char *filename, int index) {
    ...
    if(index < bcp_entries) {
        ...
        buff = sysMalloc_persistent(strlen(filename) + path_len + 14);
...
out:
        if(path != bootclasspath[index].path){
          if(path != NULL) sysFree(path);
          if(buff != NULL) sysFree(buff);
        }
    }
...
int initialiseClassStage1(InitArgs *args) {
    ...
    if(args->testmode) testing_mode = TRUE;
    if(args->persistence) is_persistent = TRUE;
    setClassPath(args);
    setBootClassPath(args);

    parseBootClassPath();
    if( stat(class_name,&st) == 0) is_hash_exist = TRUE;
...
int initialiseClassStage2() {
    ...
    if(testing_mode)jam_printf("<Class: java.lang.class is inited %p>\n",java_lang_Class);
...
int msync_class(HashTable *ptr){
  if(!ptr || !is_persistent) return FALSE;
  uintptr_t *mem = ptr->hash_table;
  uintptr_t size = alignSize(sizeof(HashEntry)*ptr->hash_size);
  *mem--;
  *mem--;
  //printf("mem:%p,size:%d\n",mem,size);
  if(msync(mem,size,MS_SYNC) == -1){
    perror("msync at class");
    return FALSE;
  }
}
