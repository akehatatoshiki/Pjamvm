...
jobject Jam_SetPersistenceObject(JNIEnv *env, jobject obj ,jstring str);
jobject Jam_GetPersistenceObject(JNIEnv *env,jstring str);
void Jam_DeletePersistenceObject(JNIEnv *env,jstring str);
static int initJNIPrefs(char *path,int exist);
/* Parameter for Persistence */
int IS_PERSISTENCE = FALSE;

int initialiseJNI(InitArgs *args) {
    struct stat st;
    /* Initialise the global reference tables */
    initJNIGrefs();
    if(args->persistence) IS_PERSISTENCE = TRUE;
...
/* ----------------- Persistence reference support ----------------- */

#define NUM_PERSISTENCE_TABLE 10
#define KEY_SIZE 256
typedef struct persistence_hash{
  Object *obj;
  char key[KEY_SIZE];
} PHash;
typedef struct persiste_ref_table{
    PHash  hash[NUM_PERSISTENCE_TABLE];
    int    has_deleted;
    VMLock lock;
} PersistenceTable;
#define PTablelength (sizeof(PersistenceTable)/PAGESIZE+1)*PAGESIZE

static PersistenceTable *p_table;

int calcHash(const char *key){
        int calc = 0;
	while(*key) calc += (calc<<5) + *key++;
	return ((calc >> 5) + calc) % NUM_PERSISTENCE_TABLE;
}

int rehash(unsigned int calc){
  return (calc+1) % NUM_PERSISTENCE_TABLE;
}

int searchHash(const char *key){
  int i = 0;
  int calc = calcHash(key);
  while(i < NUM_PERSISTENCE_TABLE){
    if(strncmp(p_table->hash[calc].key, key, KEY_SIZE) == 0 || p_table->hash[calc].key[0] == '\0') break;
    calc = rehash(calc);
    i++;
  }
  if(calc >= NUM_PERSISTENCE_TABLE) return -1;
  return calc;
}
...
static int initJNIPrefs(char *path,int exist){
  int fd;
  PersistenceTable *ptr = pMemMalloc(PTablelength,"persistence_ht",TRUE);
  if(exist != 0){
    int i;
    for(i = 0; i < NUM_PERSISTENCE_TABLE; i++){
      p_table->hash[i].obj = NULL;
      p_table->hash[i].key[0] = '\0';
      initVMLock(p_table->lock);
    }
    msync(p_table,PTablelength,MS_SYNC);
  }
  return TRUE;
}
void lockJNIPrefs(Thread *self){
    // Disabling and enabling suspension is slow,
    //   so only do it if we have to block
    if(!tryLockVMLock(p_table->lock, self)) {
        disableSuspend(self);
        lockVMLock(p_table->lock, self);
        enableSuspend(self);
    }
    fastDisableSuspend(self);
}
void unlockJNIPrefs(Thread *self){
    fastEnableSuspend(self);
    unlockVMLock(p_table->lock, self);
}
void addJNIPrefUnlocked(Object *ref,int hash,const char *key){
    p_table->hash[hash].obj = sysRealloc_persistent(p_table->hash[hash].obj,sizeof(jobject*));
    strncpy(p_table->hash[hash].key, key, KEY_SIZE);
    p_table->hash[hash].obj = &ref;
    msync(p_table,PTablelength,MS_SYNC);
}
static jobject addJNIPobj(Object *ref, jstring str){
    if(!IS_PERSISTENCE) return NULL;
    const char *key = (const char *)String2Utf8(str);
    if(*key == '\0' || ref == NULL) { sysFree((void*)key); return NULL;}
    int hash;
    hash = searchHash(key);
    if(hash == -1){
        //jam_fprintf(stderr,"set hash error \n");
        sysFree((void*)key);
        return NULL;
    }
    ...
    lockJNIPrefs(self);
    addJNIPrefUnlocked(ref, hash, key);
    unlockJNIPrefs(self);
    sysFree((void*)key);
    return OBJ_TO_REF(ref, PERSISTENCE_REF);
}

static jobject getJNIPobj(jstring str){
  if(!IS_PERSISTENCE) return NULL;
  const char *key = (const char *)String2Utf8(str);
  if(*key == '\0') { sysFree((void*)key); return NULL;}
  int hash;
  hash = searchHash(key);
  sysFree((void*)key);
  if(hash == -1 || p_table->hash[hash].obj == NULL){
    //jam_fprintf(stderr,"It cant be not found this key.\n"); return NULL;
  }
  return OBJ_TO_REF(p_table->hash[hash].obj,PERSISTENCE_REF);
}

static void delJNIPobj(jstring str){
    if(!IS_PERSISTENCE) return ;
    const char *key = (const char *)String2Utf8(str);
    if(*key == '\0') { sysFree((void*)key); return ;}
    int hash;
    hash = searchHash(key);
    sysFree((void*)key);
    if(hash == -1 || p_table->hash[hash].obj == NULL){
    ...
    p_table->hash[hash].obj = NULL;
    p_table->hash[hash].key[0] = '\0';
    msync(p_table,PTablelength,MS_SYNC);
    unlockJNIPrefs(self);
    return;
}
...
void addJNIGrefUnlocked(Object *ref, int type) {
    if(global_refs[type].next == global_refs[type].size) {
        ...
        if(global_refs[type].next + GREF_LIST_INCR > global_refs[type].size) {
            ...
            global_refs[type].table = sysRealloc_persistent(global_refs[type].table,
...
#define MARK_JNI_PERSISTENCE_OBJ()                               \
void markJNIPersistenceObjs(){                                   \
  if(IS_PERSISTENCE){                                           \
    int i;                                                      \
    for(i = 0; i<NUM_PERSISTENCE_TABLE;i++){                    \
      if(p_table->hash[i].obj != NULL){                         \
        Object *obj = (Object*)p_table->hash[i].obj;            \
        markJNIPersistenceObj(obj);                             \
      }                                                         \
    }                                                           \
  }                                                             \
}
...
/* Add persistence root */
jobject Jam_SetPersistenceObject(JNIEnv *env, jobject obj ,jstring str) {
    //return addJNIPobj(REF_TO_OBJ_WEAK_NULL_CHECK(obj),str);
    return NULL;
}

jobject Jam_GetPersistenceObject(JNIEnv *env,jstring str) {
    //return getJNIPobj(str);
    return NULL;
}
...
struct _JNINativeInterface Jam_JNINativeInterface = {
    ...
    Jam_PushLocalFrame,
    Jam_PopLocalFrame,
    Jam_SetPersistenceObject,
    Jam_GetPersistenceObject,
    Jam_DeletePersistenceObject,
    Jam_NewGlobalRef,
    Jam_DeleteGlobalRef,
    Jam_DeleteLocalRef,
    Jam_IsSameObject,
