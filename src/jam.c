            ...
            default:
                if(strcmp(argv[i], "-?") == 0 ||
                  ...
                  testmode = TRUE;
                    ...
                    /* This option only use when use valgrind */
                    args->valgrind = TRUE;
                    ...
                    args->classpath = argv[++i];
                       ...
                       if(i == argc - 1){
                         printf("%s : missing dirname\n", argv[i]);
                         goto exit;
                       }
                       if(args->dirname = argv[++i] != NULL){
                         if(mkdir(args->dirname) != 0){
                           perror("Can't create directry. Check your dirname\n");
                           goto exit;
                         }
                       }else{
...
int main(int argc, char *argv[]) {
    ...
    /*if(args.persistence == TRUE && args.dirname == NULL){
      args.dirname = "persistence_datas";
      if(mkdir(args.dirname) != 0){
        perror("Can't create directry. Probably 'persistence_datas' already exisits\n");
        exit(1);
      }
    }*/
    ...
    if(testmode) printf("initialise is complete\n");
    if((system_loader = getSystemClassLoader()) == NULL)
        goto error;

    if(testmode) printf("ClassLoader loading is complete.\n");
    ...
    if(testmode) printf("Main class:%p\n",main_class);
    if(main_class != NULL)
    ...
    if((array_class = findArrayClass(SYMBOL(array_java_lang_String))) &&
        ...
        for(; i < argc; i++)
            if(!(args[i] = Cstr2String(argv[i])))
                break;
        if(testmode) printf("Try to Execute main_class:%p %s mb:%p\n",main_class,CLASS_CB(main_class)->name,mb);
...
error:
    ...
    //if(testmode) printf("Error. Will Exit VM.\n");
