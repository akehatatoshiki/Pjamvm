...
NativeMethod lookupInternal(MethodBlock *mb) {
    ...
    //jam_printf("<DLL: Looking up %s %s internally>\n", mb->name,cb->name);
    ...
    if(native_methods[i].classname) {
        ...
        //jam_printf("<DLL: Internal method name resloved %s %s>\n",cb->name,mb->name);
        ...
        if(methods[i].methodname) {
            if(verbose)
                jam_printf("internal %p",methods[i].method);
...
int initialiseDllStage2(){
  if(is_persistent && (is_first_exp() == FALSE)) reloadDlls();
  return TRUE;
}
...
int resolveDll(char *name, Object *loader) {
    ...
    //jam_printf("<DLL: Attempting to resolve library %s>\n", name);
...
void *lookupLoadedDlls0(char *name, Object *loader) {
    ...
    //jam_printf("<DLL: Looking up %s loader %p in loaded DLL's>\n", name, loader);
...
uintptr_t *callJNIWrapper(Class *class, MethodBlock *mb, uintptr_t *ostack) {
    ...
    //jam_printf("<DLL: Calling JNI method %s.%s%s :%p>\n", CLASS_CB(class)->name,
          //mb->name, mb->type,(void*)opntr++);
    /* XXX NVM CHANGE 007.000.003 */
    if (is_persistent && is_first_exp() == FALSE){
      //resolveNativeMethod(mb);
      if(lookupLoadedDlls(mb) == NULL){
        jam_printf("<DLL: JNI Method is broken>\n");
        return NULL;
      }
    }
    if(!initJNILrefs()){
        jam_printf("<DLL return NULL>\n");
        return NULL;
    }
...
uintptr_t *callJNIWrapperRefReturn(Class *class, MethodBlock *mb,
    ...
    //jam_printf("<DLL: Calling JNI method %s.%s%s>\n", CLASS_CB(class)->name,
          //mb->name, mb->type);
    /* XXX NVM CHANGE 007.000.003 */
    if (is_persistent && is_first_exp() == FALSE){
      //resolveNativeMethod(mb);
...
NativeMethod setJNIMethod(MethodBlock *mb, void *func) {
    ...
    mb->code = func;
    //jam_printf("<DLL:setJNIMethod class:%s mb->access_flags:%d mb->code:%p >\n",CLASS_CB(mb->class)->name,mb->access_flags,mb->code);
...
NativeMethod lookupLoadedDlls(MethodBlock *mb) {
    ...
    if(func == NULL) {
        ...
        //jam_printf("<DLL: class_loader:%p>\n",loader);
        func = classlibLookupLoadedDlls(fullyMangled, loader);
        //jam_printf("<DLL:lookupLoadedDlls func:%p>\n",func);
