...
int initialiseJNI(int is_p) {
    if(is_p) is_persistent = TRUE;
...
void addJNIGrefUnlocked(Object *ref, int type) {
    if(global_refs[type].next == global_refs[type].size) {
        ...
        if(global_refs[type].next + GREF_LIST_INCR > global_refs[type].size) {
            ...
            global_refs[type].table = sysRealloc(global_refs[type].table,global_refs[type].size * sizeof(Object*));
        }
    }
...
jclass Jam_GetObjectClass(JNIEnv *env, jobject obj) {
    //jam_printf("Can attach classname?:%p\n",CLASS_CB(REF_TO_OBJ(obj)->class)->name);
    if(CLASS_CB(REF_TO_OBJ(obj)->class)->name == NULL){
      jam_printf("classname is broken. class addr: %p\n",CLASS_CB(REF_TO_OBJ(obj)->class));
    }
    //ClassBlock *cb = CLASS_CB(REF_TO_OBJ(obj)->class);
    //jam_printf("Class is %s, addr is : %p\n",cb->name,REF_TO_OBJ(obj)->class);
    //jam_printf("fields:%p fields_count:%d fields->u:%p u.static_value.p:%p u.static_value.data:%p\n",
                //cb->fields,cb->fields_count,cb->fields->u,cb->fields->u.static_value.p,cb->fields->u.static_value.data);
