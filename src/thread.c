...
void uncaughtException() {
    ...
    if(uncaught_mb != NULL) {
        ...
        if(exceptionOccurred()){
            jam_printf("Exception occurred at uncaughtException: %s \n",CLASS_CB(excep->class)->name);
            setException(excep);
          }
    }
...
int initialiseThreadStage1(InitArgs *args) {
    ...
    //if(testing_mode) jam_printf("initialise Thread Stage1 done\n");
    return TRUE;
}
...
int initialiseThreadStage2(InitArgs *args) {
    ...
    //if(testing_mode) jam_printf("initialise Thread Stage2\n");
