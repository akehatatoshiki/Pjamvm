...
MethodBlock *findMethod(Class *class, char *methodname, char *type) {
   ...
   //jam_printf("findMethod addr:%p,name:%s\n",mb,cb->name);
   int i;
...
retry:
    ...
    //jam_printf("resolved method md:%p name:%s\n",mb,CLASS_CB(class)->name);
    return mb;
}
...
MethodBlock *lookupVirtualMethod(Object *ob, MethodBlock *mb) {
    ...
    if(!mb)jam_printf("<mb:%p>\n",mb);
    //else jam_printf("<mb:%p mb_name:%s cb:%s mb_class:%s>\n",mb,mb->name,cb->name,CLASS_CB(mb->class)->name);
