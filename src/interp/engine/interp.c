...
uintptr_t *executeJava() {
   ...
   /* Caching is supported in all variants and
    ...
    PREPARE_MB(mb);
    pc = (CodePntr)mb->code;
    //if((unsigned long)pc < 0x7f65ac16f000) jam_printf("pc not P.name:%s,addr:%p\n",mb->name,mb->code);
...
#define PUTFIELD_QUICK(type, suffix)                        \
    ...
    DEF_OPC_210(OPC_INVOKEINTERFACE_QUICK, {
        ...
        //jam_printf("is new_mb valid? addr:%p\n",new_mb);
        goto invokeMethod;
    })
    ...
    DEF_OPC_210(OPC_INVOKEVIRTUAL_QUICK, {
        ...
        //jam_printf("is class valid? addr:%p(DEF_OPC_210)\n",new_class);
        new_mb = CLASS_CB(new_class)->method_table[INV_QUICK_IDX(pc)];
        //jam_printf("is new_mb valid? addr:%p name:%p(DEF_OPC_210)\n",new_mb,CLASS_CB(new_class)->name);
        goto invokeMethod;
    })
...
invokeMethod:
{
    ...
    //jam_printf("arg1:%p,new_mb:%s\n",arg1,CLASS_CB(new_mb->class)->name);
    ...
    if(new_mb->access_flags & ACC_NATIVE) {
        //if(!is_first_exp()) jam_printf("inovoker %p %p\n",new_mb->native_invoker,&resolveNativeWrapper);
        //if(!is_first_exp()) new_mb->native_invoker = &resolveNativeWrapper;
        ...
        pc = (CodePntr)mb->code;
        cp = &(CLASS_CB(mb->class)->constant_pool);
        //jam_printf("invoke done..\n");
    }
    //jam_printf("handler:%p\n",pc->handler);
    DISPATCH_FIRST
}
