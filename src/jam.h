...
// For Persistence //
extern void *pMemMalloc(int n, char* name, int create_file);
extern void *pMemRealloc(void *addr, int size, char* name, int create_file);
extern void expandNVM();
extern void *sysMalloc_persistent(int size);
extern void sysFree_persistent(void* addr);
extern void *sysRealloc_persistent(void *addr, int size);
extern void calldoMark();
...
extern void gcPendingNVMFree(void *addr);
...
extern void markPersistenceObj();
...
extern int reinitialiseNativeMethods();
...
extern char *jam_strcat(char *s1,char *s2);
...
extern void *getHashedNativeMethodUtf8();
...
#define findNativeMethodUtf8(string) \
    findHashedNativeMethodUtf8(string, FALSE)

#define newNativeMethodUtf8(string) \
    findHashedNativeMethodUtf8(string, TRUE)
...
extern int initialiseDllStage2();
