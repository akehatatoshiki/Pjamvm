...
void setException(Object *exp) {
    //jam_printf("Exception set:%s \n",CLASS_CB(exp->class)->name);
...
void signalChainedExceptionClass(Class *exception, char *message,
    ...
    if(exp && init) {
        ...
        //jam_printf("Exception occurred at signalChainedExceptionClass: %s. message: %s \n",CLASS_CB(exp->class)->name,message);
        setException(exp);
    }
}
