...
static int IS_IMAGE_EXIST = FALSE;

/* Persistence HashTable */
#define KEY_SIZE 256
#define HASH_SIZE 10
...
static int IS_HASH_EXIST = FALSE;
...
static int fd_img,fd_hash,heapsize;
...
static void **pending_free_nvmlist = NULL;
...
int initHash(int fd){
  if((fd = initFiles(fd,hashsize)) == -1) return -1;
...
int initialiseNVM(){
  ...
  nvmCurrentSize = alignSize(nvmCurrentSize);
	if( stat(nvmpath,&st) == 0) {
		if((nvm_fd = open (nvmpath, O_RDWR | O_APPEND , S_IRUSR | S_IWUSR))== -1){
      		perror("Couldn't create memory image files. Check your image path.");
      		return FALSE;
    	}
    IS_NVMIMAGE_EXIST = TRUE;
	}else{
		if((nvm_fd = open (nvmpath, O_RDWR | O_CREAT , S_IRUSR | S_IWUSR))== -1){
      		perror("Couldn't create memory image files. Check your image path.");
      		return FALSE;
    	}
    if((nvm_fd = initFiles(nvm_fd,nvmCurrentSize)) == -1) return FALSE;
	}
	nvm = (char*) mmap(NVMAddr, nvmCurrentSize, PROT_READ|PROT_WRITE, MAP_SHARED, nvm_fd, 0);
  //printf("%p,%d\n",nvm,nvmCurrentSize);
	if(msync(nvm, nvmCurrentSize, MS_SYNC) == -1){
    printf("%d\n",errno);
    perror("msync");
    return FALSE;
  }
  ...
  //printf("NVM init done?\n");
	...
	if (testing_mode == TRUE){
		jam_printf("Created NVM Memory an %p with size %d\n", nvm-sizeof(OPC), nvmCurrentSize);
	}
	/* 1st chunk =  all mem */
	nvmfreelist = (nvmChunk*) nvm;
	if (IS_NVMIMAGE_EXIST == FALSE){
...
int initialiseAlloc(InitArgs *args) {
  ...
  if(args->persistence){
    ...
    is_persistent = TRUE;
    currentdir = args->dirname;
    char *imgpath = "Heapimage";
    ...
    if(IS_HASH_EXIST){
      ...
      hash = (PHash *)mmap(0,hashsize,PROT_READ|PROT_WRITE, MAP_SHARED, fd_hash, 0);
      if(hash == MAP_FAILED){
      	perror("error occourd at init Phash");
        return -1;
      }
      ...
      hashsize = alignSize(sizeof(PHash)*HASH_SIZE);
      if((fd_hash = open(hashpath,O_RDWR|O_CREAT,S_IRUSR|S_IWUSR)) == -1){
        perror("Couldn't create memory image files. Check your image path.");
        return FALSE;
      }
    ...
    if(IS_IMAGE_EXIST == TRUE){
      first_ex = FALSE;
      if((fd_img = open(imgpath,O_RDWR|O_APPEND,S_IRUSR|S_IWUSR)) == -1){
        perror("Couldn't create memory image files. Check your image path.");
        return FALSE;
      }
    }else{
      ...
      if((fd_img = initFiles(fd_img,heapsize)) == -1) return FALSE;
    }
    ...
    //if(testing_mode) jam_printf("HDR_ALLOCED  %d\n", HDR_ALLOCED(HEADER(heapbase)));
    ...
    if(testing_mode) jam_printf("Initialise alloc Done\n");
    return TRUE;
}
...
void markPersistenceObj(){
  int i = 0;
  while(i < HASH_SIZE){
    Object *ptr = (Object*)hash[i].val;
    if(ptr != NULL){
      markConservativeRoot(ptr);
      markChildren(ptr,TRUE,TRUE);
    }
    i++;
  }
}
    ...
    for(mark_scan_ptr = heapbase; mark_scan_ptr < heaplimit;) {
        ...
        if(HDR_ALLOCED(hdr)) {
            ...
            if(mark) {
                num++;
                sum_size += size;
                //if(testing_mode && CLASS_CB(ob->class)->name) jam_printf("object marked:%s\n",CLASS_CB(ob->class)->name);
...
static uintptr_t doSweep_heapmax(Thread *self) {
    char *ptr;
    Chunk newlist;
    Chunk *curr = NULL, *last = &newlist;
...
Class *allocClass() {
    ...
    if(class != NULL) {
      ...
      jam_printf("Cant alloc class object\n");
    }
...
unsigned long valgrind_addr(unsigned long addr,char* str){
  ...
  if(valgrind){
    if(strcmp(str,"utf8_ht") == 0){
        return 0x4041008;
    }else if(strcmp(str,"string_ht") == 0){
        return 0x4036000;
    }else if(strcmp(str,"classes_ht") == 0){
        return 0x4031000;
    }else if(strcmp(str,"bootCl_ht") == 0){
        return 0x408a000;
    }else if(strcmp(str,"bootPck_ht") == 0){
        return 0x4031000;
    }else if(strcmp(str,"temp") == 0){
        return 0x4087000;
    }else if(strcmp(str,"nativemethods_ht") == 0){
        return 0x4062008;
    }else{
        return addr;
    }
...
void *pMemMalloc(int n, char* str, int create_file) {
	uintptr_t size = n + sizeof(uintptr_t);
	uintptr_t *mem,*tmp;
	int fd;

	if (is_persistent && create_file){
    //char *str = jam_strcat(currentdir,name);

    struct stat st;
		unsigned long buffer[10];
		size = alignSize(size + sizeof(unsigned long));
		if( stat( str, &st ) == 0 ){
      // File Exist!
			if((fd = open(str, O_RDWR | O_APPEND , S_IRUSR | S_IWUSR)) == -1 ||
          read(fd, &buffer, sizeof(unsigned long)+sizeof(uintptr_t)) == -1){
        perror("open at pMemMalloc");
      }
      size = buffer[1];
			mem = (uintptr_t*)mmap(valgrind_addr((unsigned long)buffer[0],str),size, PROT_READ|PROT_WRITE,MAP_SHARED,fd, 0);
      if(mem != MAP_FAILED){
        *mem++ = (unsigned long)mem;
        //*mem++ = size;

      }else{
        jam_printf("try to set addr %p,size %d\n",(unsigned long)buffer[0],buffer[1]);
        perror("mmap at pMemMalloc");
        exitVM(1);
      }
      if(testing_mode) jam_printf("pMemMalloc File Exist:%s\n",str);
		}else{
      fd = initFiles(open(str, O_RDWR | O_CREAT , S_IRUSR | S_IWUSR),size-1);
      if(fd != -1){
		      mem = (uintptr_t*)mmap(valgrind_addr(0,str), size, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
          tmp = mem;
		      if(mem != MAP_FAILED){
            if(testing_mode)jam_printf("mem:%p ,%p\n",(unsigned long)tmp,(unsigned long)mem);
            *mem++ = (unsigned long)mem;
            //*mem++ = size;
      ...
      if(msync(tmp,size,MS_SYNC) != 0) perror("Msync");
		}
 ...
 */
void *pMemRealloc(void *addr, int size, char* name, int create_file) {
	if(addr == NULL)
		return pMemMalloc(size, name, FALSE);
	else {
		uintptr_t *mem = addr;
		uintptr_t old_size = *--mem;
		uintptr_t new_size = size + sizeof(uintptr_t);
		...
		if(old_size/sys_page_size == new_size/sys_page_size) {
			*mem = new_size;
			...
			uintptr_t copy_size = new_size > old_size ? old_size : new_size;
			void *new_mem = pMemMalloc(size, name , FALSE);
...
void gcPendingFree(void *addr) {
    if(addr != NULL) {
        if(is_persistent && (unsigned long)addr > 0x7f65ac16f000){
          gcPendingNVMFree(addr);
        }else{
...
void gcPendingNVMFree(void *addr){
    if(addr == NULL) return;
    if(is_persistent){
      *(void**)addr = pending_free_nvmlist;
      pending_free_nvmlist = addr;
    }else{
      gcPendingFree(addr);
    }
}
...
void freePendingFrees() {
    ...
    if(is_persistent){
      while(pending_free_nvmlist) {
          void *next = *pending_free_nvmlist;
          sysFree_persistent(pending_free_nvmlist);
          pending_free_nvmlist = next;
      }
    }
}
...
void expandNVM(){
	...
	nvmChunk *new;
	nvmChunk *chunk;
	nvmCurrentSize = nvmCurrentSize + alignSize(INCREASE_VALUE);
  ...
  if(write(nvm_fd,"",1) == -1){
	...
	nvm = (char*) mremap(nvm, oldSize, nvmCurrentSize, 1|2, 0);
...
void *sysRealloc_persistent(void *addr, int size){
  //if(testing_mode) jam_printf("sysRealloc_p:%p\n",addr);
	void *mem;
	if (is_persistent && addr != NULL){
		/*	chunk = ptr - header */
...
void *sysRealloc(void *addr, int size) {
	//if(testing_mode) jam_printf("sysRealloc:%p\n",addr);
...
void sysExit(){
  ...
  if(is_persistent  &&
     ...
     msync(nvm-sizeof(OPC),nvmCurrentSize,MS_SYNC) == 0){
       if(close(fd_img) == -1 || close(nvm_fd) == -1 || close(fd_hash) == -1) perror("close");
  }else{
    printf("%d\n",errno);
